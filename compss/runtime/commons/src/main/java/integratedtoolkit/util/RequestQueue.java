...
/**
 * The RequestQueue class is an utility to enqueue requests from a certain type.
 * Any component can add a Request to the queue at the end or priorize the 
 * treatment of that request by adding it to the head of the queue. At any 
 * point of the execution a thread can dequeue a Request from the queue to treat 
 * it, if there are no requests on the queue it falls asleep until a new 
 * request is enqueued.
...
public class RequestQueue<T> {
    ...
    /**
     * Constructs a new RequestQueue without any pending request nor asleep 
    ...
    /**
     * Dequeues a request from the queue. If there are no pending requests, the 
     * current Thread falls asleep until a new Request is added. This request 
     * will be return value of the method.
